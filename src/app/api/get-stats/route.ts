...
const getStats = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const groupBySubject = (questions: QuestionDetail[]) => {
      return questions.reduce((acc: Record<string, QuestionDetail[]>, question) => {
        const subjectId = question.questionObjectId.toString();
        if (!acc[subjectId]) {
          acc[subjectId] = [];
        }
    ...
    const userRatingPercentile = await calculatePercentile(
      ...
      user.selectedSubjects.length > 0 ? user.selectedSubjects[0].userRating : 0
    );
