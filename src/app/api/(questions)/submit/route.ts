...
import UserModel, { SelectedSubjectsAndStats } from "@/models/user.model";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    user.questionsSolvedDetails.push({
      ...
      subjectCode: subjectObjectId.toString(), // Changed from subjectCode to subjectObjectId
      userAnswer,
      userQuestionTime,
      isCorrect,
    ...
    const subjectStatsAggregation = await UserModel.aggregate([
      { $match: { _id: new mongoose.Types.ObjectId(userId) } },
      { $unwind: "$selectedSubjects" },
      { $match: { "selectedSubjects.subjectObjectId": new mongoose.Types.ObjectId(subjectObjectId) } },
      { $limit: 1 },
      { $project: { selectedSubjects: 1 } }
    ]);

    let subjectStats = subjectStatsAggregation.length > 0 ? subjectStatsAggregation[0].selectedSubjects : null;
    ...
    if (subjectStats) {
      ...
      user.selectedSubjects.push({
        subjectObjectId: new mongoose.Types.ObjectId(subjectObjectId),
        subjectName,
        subjectCode: subjectObjectId.toString(), // Changed from subjectCode to subjectObjectId
